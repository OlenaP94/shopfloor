...
## The second layer: what the classifier cannot say

A classifier knows four components and nothing else. Shown a burst hose it will still name
a valve grade, because that is the only vocabulary it has. So `make anomaly` trains a
convolutional autoencoder as a second layer, which answers a different question: does this
cycle belong to the population I was trained on at all?

The mechanism is the bottleneck. 24 × 600 values have to pass through 16 × 75 and come
back — a factor of twelve. Only structure the network has seen often is worth spending
capacity on, so anything unfamiliar returns distorted.

Trained on the 559 training cycles with a healthy cooler, **without labels**, threshold set
at the 99th percentile of its own training error:

| Cooler | Cycles | Mean error | Flagged |
|---|---|---|---|
| 3 — close to total failure | 111 | 1.473 | 100 % |
| 20 — reduced efficiency | 92 | 0.311 | 100 % |
| 100 — full efficiency | 91 | 0.028 | 0 % |

AUC for separating a degraded cooler by reconstruction error alone: **1.000**.

### The control that decides what that is worth

An AUC of 1.000 on the easiest component proves very little on its own — the cooler shifts
oil temperature across eight of the twenty-four channels, so its degraded grades are
grossly unfamiliar. The question that matters is whether the detector reacts to
*unfamiliarity* or merely to *something being wrong*.

The other three components varied freely in that training set, so the autoencoder has seen
every one of their grades and should not separate them:

| Component | Grades seen in training | AUC | |
|---|---|---|---|
| Cooler | `[100]` | 1.000 | held out, flagged |
| Valve | `[73, 80, 90, 100]` | 0.472 | seen, no separation |
| Pump leakage | `[0, 1, 2]` | 0.559 | seen, no separation |
| Accumulator | `[90, 100, 115, 130]` | 0.622 | seen, but see below |

The accumulator's 0.622 is well outside chance for 294 cycles, so it needs explaining, and
there are two candidates. Either the detector reacts to *rarity* rather than
unfamiliarity, or degraded accumulators are genuinely harder to compress.

Rarity is ruled out by direction. In that training subset the **worst** accumulator grade
is the **most common** — 90 accounts for 48 % of cycles, and degraded grades together are
81 %. If rarity drove reconstruction error, degraded cycles would reconstruct *better* and
the AUC would fall below 0.5. It is above.

What remains is physical: a worn accumulator stops damping pressure pulsations, and
high-frequency content is exactly what a twelvefold bottleneck cannot afford to keep. The
same phenomenon that made `PS1_std` the most informative channel in the supervised
analysis shows up here from the other side, in a model that never saw a label.

So the claim is narrow and defensible: the detector fires on conditions absent from its
training data and barely at all on conditions present in it. That specificity — not the
AUC of 1.000 — is the result worth having, because a detector that flags everything
abnormal-looking would be useless as an escalation trigger.

One limit stated plainly: the dataset contains no fault outside the four labelled
components, so "detects unknown faults" is not something this experiment can demonstrate.
Holding one known component out is a proxy for it.
...
make anomaly   # trains the autoencoder and scores it by cooler grade
...
src/shopfloor/anomaly.py    autoencoder for conditions outside the training population
...
The modules form two layers that change at different rates. The **task** layer — `data`,
`dataset`, `arrays`, `features`, `splits`, `metrics` — defines what is measured and how.
The **model** layer — `baseline`, `net`, `train`, `anomaly` — does the measuring. The
arrow points one way, because a metric that depended on a model would stop numbers from
two models being comparable, and nothing would fail loudly enough to notice.
`tests/test_layers.py` enforces that direction by parsing imports, and also refuses to
let a new module exist without being assigned to a layer.
